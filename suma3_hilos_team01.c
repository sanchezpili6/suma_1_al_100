...
//Declarando semáforo
sem_t bin_one, bin_two, bin_three;

//Variable de precompilación para nuestro número
#define limit_num 1000000000
#define fragments_num 3

//Variable bandera
int readyt1 = 0, readyt2 = 0, readyt3 = 0;
long double acum = 0;
...
int main(){
  ...
  printf("Running three threads\n");  

  sem_post(&bin_one);
  sem_post(&bin_two);
  sem_post(&bin_three);

  while(readyt1 == 0 || readyt2 == 0 || readyt3 == 0){
    // printf("Calculating...\n");
  }
...
void *thread_sum3(void *arg){
  ...
  int contador;
	long double total = 0;
  for (contador = (limit_num / fragments_num) * 2; contador <= ((limit_num / fragments_num) * 3)+1; contador++) {
    total +=contador;
  }
  acum += total;
  // printf("Total 2: %.0Lf\n total so far: %.0Lf\n", total, acum);
